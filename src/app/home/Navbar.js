        ...
        <div className="flex flex-col py-4 px-6 h-full">
          ...
          {menuItems.map((item, index) => (
            <React.Fragment key={index}>
              <button className="block w-full text-left px-6 py-3 text-lg font-semibold tracking-wide hover:bg-gray-800 hover:text-purple-500" onClick={() => {
                if (item.dropdown) {
                  if (item.label === "Call for Papers") setIsPapersDropdownOpen(!isPapersDropdownOpen);
              ...
              {item.dropdown && (
                <div className={`pl-6 ${((item.label === "Call for Papers" && isPapersDropdownOpen) || (item.label === "Venue" && isVenueDropdownOpen) || (item.label === "Previous Years" && isprevyeardropdownopen)) ? "block" : "hidden"}`}>
                  {item.dropdown.map((dropdownItem, idx) => (
