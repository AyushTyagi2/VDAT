...
const Carousel = ({ images, currentImageIndex, setCurrentImageIndex, swipeHandlers }) => {
  return (
    <div
      ...
      <div className="flex-grow flex items-center justify-start z-10 relative px-6 md:px-28">
        <div className="max-w-3xl text-center md:text-left">
          <div className="mb-6 inline-block">
            <span className="px-4 py-1 bg-yellow-400 text-black font-medium text-sm rounded-full inline-block mb-3 animate-pulse">
              August 7-9, 2025
            </span>
          </div>
          ...
          <p className="text-lg lg:text-2xl text-white mb-8 md:mb-10 font-medium drop-shadow-lg">
      ...
      {comingSoon ? (
        <div className="text-center">
          <div className="inline-block bg-purple-100 rounded-xl p-8 shadow-md transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
            <h3 className="text-2xl font-semibold text-purple-600 mb-2">Coming Soon!</h3>
            <p className="text-gray-600">
              We're finalizing our {title.toLowerCase()}. Check back later for updates.
            </p>
          </div>
        </div>
      ) : (
...
const Timeline = () => {
  ...
  const dates = [
    { date: "May 15, 2025", event: "Paper Submission Deadline", description: "Deadline for all paper submissions. No extensions will be granted." },
    { date: "June 10, 2025", event: "Notification of Acceptance", description: "Authors will be notified about the status of their submissions." },
    { date: "July 1, 2025", event: "Camera-ready Submission", description: "Final papers must be submitted in the required format." },
    { date: "July 15, 2025", event: "Early Registration Deadline", description: "Last day to register at the discounted early bird rate." },
    { date: "August 7-9, 2025", event: "Conference Dates", description: "Main conference events and presentations." }
  ];
  ...
  useEffect(() => {
    const findNextEvent = () => {
      for (let i = 0; i < dates.length; i++) {
        const eventDate = new Date(dates[i].date.replace("August 7-9, 2025", "August 7, 2025"));
  ...
  const isPastDate = (dateString) => {
    const eventDate = new Date(dateString.replace("August 7-9, 2025", "August 7, 2025"));
        ...
        <div className="max-w-4xl mx-auto">
          {dates.map((item, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`flex relative pb-16 transition-all duration-300 ${isHighlighted ? 'scale-105' : ''}`}
              ...
              >
                ...
                {index !== dates.length - 1 && (
                  <div className="absolute inset-0 flex items-center justify-center h-full w-10">
                    <div className={`h-full w-1 ${isPast ? 'bg-green-400' : 'bg-yellow-400'} opacity-30 pointer-events-none`}></div>
                  </div>
                )}
                
                {/* Circle */}
                <div 
                  className={`flex-shrink-0 w-10 h-10 rounded-full ${isPast ? 'bg-green-500' : isHighlighted ? 'bg-yellow-300' : 'bg-yellow-400'} 
                    inline-flex items-center justify-center text-white relative z-10 shadow-lg
                    transition-all duration-300 ${isHighlighted ? 'scale-125' : ''}`}
                ...
                {/* Content */}
                <div className={`flex-grow pl-6 ${isHighlighted ? 'bg-purple-800 bg-opacity-40 p-4 rounded-lg' : ''}`}>
                  <div className="flex items-center mb-1">
                    <h3 className="text-xl md:text-2xl font-semibold">{item.event}</h3>
                    {isHighlighted && <span className="ml-2 px-2 py-1 bg-yellow-500 text-xs font-bold rounded-full">NEXT</span>}
                  </div>
                  <div className="flex items-center text-purple-200 mb-2">
                    <Clock className="h-4 w-4 mr-2" />
                    <p>{item.date}</p>
                  </div>
                  <p className={`text-sm md:text-base ${isHighlighted ? 'text-purple-100' : 'text-purple-200'} transition-colors duration-300`}>
                    {item.description}
                  </p>
                </div>
              </div>
...
const HomePage = () => {
  ...
  return (
    ...
    >
      ...
      <SponsorsSection title="Our Sponsors" items={sponsors} comingSoon={true} />
      
      <SponsorsSection title="Endorsed By" items={endorsedBy} comingSoon={true} />
