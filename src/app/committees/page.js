...
const CommitteeCard = ({ title, members }) => {
  return (
    <div className="bg-white  shadow-lg rounded-lg p-6 w-full md:w-1/2 lg:w-1/3 transition-transform hover:scale-105">
      <h2 className="text-xl font-bold text-indigo-700 mb-4 text-center border-b pb-2">{title}</h2>
      <ul className="space-y-3">
        {members.map((member, index) => {
          const isLast = index === members.length - 1;
          return (
            <li key={index} className="text-gray-800 text-center relative group cursor-pointer">
              <div className="relative inline-block">
                ...
                <div
                  ...
                  } bg-black shadow-md rounded-lg p-2 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                >
                  {member.linkedin && (
                  ...
                  {member.website && (
                    <a href={member.website} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:underline">
                      Website
                    </a>
                  )}
                </div>
...
const SpeakersSection = () => {
  return (
    <div>
      <Navbar />
      <header className="text-center bg-gradient-to-r from-purple-500 to-indigo-600 text-white py-40 relative"
